...
func Accesscontextmanager_accesspolicies_setiampolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateAccesscontextmanager_accesspolicies_setiampolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_resource_setIamPolicy",
		mcp.WithDescription("Sets the IAM policy for the specified Access Context Manager access policy. This method replaces the existing IAM policy on the access policy. The IAM policy controls the set of users who can perform specific operations on the Access Context Manager access policy."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Accesscontextmanager_accesspolicies_setiampolicyHandler(cfg),
	}
}
